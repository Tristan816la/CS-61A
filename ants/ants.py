    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if not self.ant.can_contain(insect) and insect.can_contain(self.ant):
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        temp = self.place
        count = 0
        while len(temp.bees) == 0 and count < self.max_range:
             temp = temp.entrance
             count += 1
             if temp is None:
                 return None
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        ...
        temp = self.place
        count = 0
        while len(temp.bees) == 0 or count < self.min_range:
             temp = temp.entrance
             count += 1
             if temp is None:
                 return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bee_copies = list(self.place.bees)
        self.armor -= amount
        if self.armor <= 0:
            self.place.remove_insect(self)
            for bee in bee_copies:
